...
(function (define, angular) {
    ...
    function startCtr($scope, $state, $timeout, $q, logger, toastr, globalService, sharepointJsom) {
        ...
        function fnLoadItem(list, node) {
            ...
            if (globalService.isUndefinedOrNull(list.hasFieldIsSend)) {
                ...
                sharepointJsom.getFields(list.title).then(function (fields) {
                    var n = fields.length;
                    for (var i = 0; i < n; i++) {
                        if (fields[i].internalName == 'IsSend' && fields[i].fieldType === SP.FieldType.prototype.boolean) {
                            hasFieldIsSend = true;
                            break;
                        }
                    }
        ...
        function fnLoadItem(list, node) {
            ...
            if (globalService.isUndefinedOrNull(list.hasFieldIsSend)) {
                ...
                sharepointJsom.getFields(list.title).then(function (fields) {
                    var n = fields.length;
                    for (var i = 0; i < n; i++) {
                        if (fields[i].internalName == 'FileStatus' && fields[i].fieldType === SP.FieldType.prototype.choice)/*SP.FieldType.prototype*/ {
                            hasFieldIsSend = true;
                            break;
                        }
                    }
        ...
        function fnGetItemOfList(list, node) {
            ...
            var include = 'Include(ID,Title, LinkFilename, FileRef, FileLeafRef, File, FileSystemObjectType)';
        ...
        function fnBindToArr(listEnumerator) {
            ...
            while (listEnumerator.moveNext()) {
                ...
                if (item.get_fileSystemObjectType() === SP.FileSystemObjectType.folder) {
                    arr.push({ listTitle: vm.data.selectedList.title, hasFieldIsSend: vm.data.selectedList.hasFieldIsSend, id: item.get_item("ID"), title: item.get_item('LinkFilename'), serverRelativeUrl: item.get_item('FileRef'), fileSystemObjectType: item.get_fileSystemObjectType() });
        ...
        function fnCopyFileToFtp(item) {
            var d = $q.defer();
            var url = _spPageContextInfo.webServerRelativeUrl == "/" ? "" : _spPageContextInfo.webServerRelativeUrl;
            $.ajax({
                type: "GET",
                url: url + '/_vti_bin/T5Service/UploadService.svc/Upload',
                contentType: "application/json; charset=utf-8",
                data: { "secureID": vm.securityStore, "url": item.serverRelativeUrl },
                dataType: 'json',
                success: function (msg) {
                    //d.resolve(msg);
                    d.resolve({ isSuccess: true, msg: msg, item: item });
                },
                error: function (err) {
                    //d.reject(err);
                    d.resolve({ isSuccess: false, msg: err, item: item });
                }
            });
            return d.promise;
        }
        function fnCopyToFtp() {
            ...
            $q.all(arrfn).then(function (results) {
                var n = results.length;
                //for (var i = 0; i < n; i++) {
                //    if (results[i].isSuccess == false)
                //        toastr.error(results[i].msg.Message);
                //    else {
                //        toastr.success(results[i].msg.Message);
                //    }
                //}
                var arrUpdateItem = [fnUpdateStatusToFile(results)];
                for (var i = 0; i < n; i++) {
                    if (items[i].hasFieldIsSend) {
                        //arrUpdateItem.push(sharepointJsom.updateItem(items[i].listTitle, items[i].id, [{ field: 'IsSend', value: true }]));
                        //{field:'internalName', value: value want to update}
                        arrUpdateItem.push(sharepointJsom.updateItem(items[i].listTitle, items[i].id, [{ field: 'FileStatus', value: 'SENT' }]));
                    }
                }
                $q.all(arrUpdateItem).then(function (statusResults) {
        ...
        function fnUpdateStatusToFile(results) {
            var n = results.length;
            var data = [];
            for (var i = 0; i < n; i++) {
                data.push({
                    url: results[i].item.serverRelativeUrl,
                    fileName: results[i].item.title,
                    status: results[i].isSuccess == true ? "Success" : "Fail",
                });
            }
            var d = $q.defer();
            var url = _spPageContextInfo.webServerRelativeUrl == "/" ? "" : _spPageContextInfo.webServerRelativeUrl;
            $.ajax({
                type: "POST",
                url: url + '/_vti_bin/T5Service/UploadService.svc/UpdateStatusFile',
                contentType: "application/json; charset=utf-8",
